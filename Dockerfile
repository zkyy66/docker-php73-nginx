...
ENV APP_ENV=${app_env:-"prod"} \
    ...
    PHPYAF_VERSION=3.0.8 \
    PHP_PHALCON_VERSION=3.4.5 \
    PHP_XDEBUG_V=2.8.0

RUN apt-get update \
    && apt-get install -y --no-install-recommends \
        curl wget vim git zip unzip less procps lsof tcpdump htop openssl supervisor \
        libz-dev \
        libssl-dev \
        libnghttp2-dev \
        libpcre3-dev \
...
##安装yaf框架扩展
ADD ext_dir/yaf-yaf-${PHPYAF_VERSION}.tar.gz /tmp
RUN cd /tmp/yaf-yaf-${PHPYAF_VERSION} \
    && phpize \
    && ./configure \
    && make \
    && make install \
...
##安装phalcon框架
ADD ext_dir/phalcon.tar.gz /tmp
RUN cd /tmp/cphalcon-${PHP_PHALCON_VERSION}/build \
    && ./install \
    && rm -rf /tmp/phalcon.tar.gz \
